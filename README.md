...
## Declare attribute namespace

js-template template attributes are properly namespaced. The default namespace (without a namespace declaration in the HTML document) is `jst`. (Although using an alternative namespace is theoretically possible, it seems to be infeasible to discover what namespace is being used in Internet Explorer 8 or below, so for now, use the standard namespace of `jst`.)

To declare the js-template namespace, attach the following attribute to the `<html>` or `<body>` element of the document:

```html
<html xmlns:jst="https://github.com/toddfast/js-template/">
```

Then, the usage of these attributes then looks like the following:
